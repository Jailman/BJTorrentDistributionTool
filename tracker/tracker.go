...
import (
	...
	bencode "github.com/Jailman/BJTorrentDistributionTool/bencode-go"
)
...
//发送任务状态
func handleConnection_SendStatus(status string) {
	configPath := "./btmasterconfig.yaml"
	config, err := ReadYaml(configPath)
	if err != nil {
		fmt.Println("Read yaml error:", err)
		return
	}
	masterlisten := config.MasterAddr
	tcpAddr, err := net.ResolveTCPAddr("tcp4", masterlisten)
	if err != nil {
	...
	sendstatus := "{\"Mission\":\"Tracker\", \"Status\":\"" + status + "\"}"
	log.Printf(sendstatus)
	conn.Write(protocol.Enpack([]byte(sendstatus)))

	log.Printf("Status sent.")
	defer conn.Close()
...
func (t *trackerTorrent) handleAnnounce(now time.Time, peerListenAddress *net.TCPAddr, params *announceParams, response bmap) (err error) {
	...
	case "completed":
		...
		handleConnection_SendStatus(strconv.Itoa(int(t.downloaded)))
	case "stopped":
